...
class WhitelistManager {
  ...
  loadWhitelist() {
    try {
      if (fs.existsSync(this.filePath)) {
        ...
        console.log(`✅ Loaded ${this.whitelist.size} users from whitelist`);
      } else {
      ...
      if (fs.existsSync(this.filePath)) {
        try {
          ...
          if (existing && !Array.isArray(existing)) {
            ...
            metadata.created = existing.created || new Date().toISOString();
  ...
  isWhitelisted(userId) {
    return this.whitelist.has(userId.toString());
  }
